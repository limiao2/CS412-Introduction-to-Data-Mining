...
{                               /* --- add an item set to a c/m tree */
  ...
  node->item    = i;            /* store the current/last item */
  node->supp    = supp;         /* and the support of the item set */
  ...
  *p = node;                    /* into the sibling list */
  while (--n >= 0) {            /* traverse the rest of the items */
    ...
    if (!node) return -1;       /* create a new prefix tree node */
    node->item    = *items++;   /* store the current/last item */
    node->supp    = supp;       /* and the support of the item set */
...
CMTREE* cmt_proj (CMTREE *dst, CMTREE *src, ITEM item)
...
{                               /* --- print a c/m prefix tree */
  ...
  if (base) printf("%s/", ib_name(base, cmt->item));
  printf("%"ITEM_FMT, cmt->item);  /* print item name if known */
  printf(" (%"RSUPP_FMT")\n", cmt->max);
